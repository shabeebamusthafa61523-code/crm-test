...
const DeveloperDashboard = () => {
  ...
  const devUserTasks = useMemo(() => {
    ...
    tasks.forEach(t => {
      ...
      const aId  = aObj ? (aObj.id || aObj._id) : t.assigned_to;
      const name = aObj?.name || (() => {
        const f = allUsers.find(u => String(u._id || u.id) === String(aId));
        return f?.name || `Dev #${String(aId || '').slice(0,5)}`;
      })();
  ...
  return (
    <div >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
        {[
          { icon: Braces,     label: 'Total Team Tasks', value: tasks.length,                               color: '#6366f1' },
