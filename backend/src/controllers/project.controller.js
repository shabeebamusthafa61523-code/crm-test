...
export const getProjects = async (req, res) => {
  try {
    ...
    if (assignedEmployee) query.assignedEmployees = assignedEmployee;
...
export const getProjectById = async (req, res) => {
  try {
    ...
    const tasks = await Task.find({ project: id }).populate('assignedTo', 'name email avatar').sort({ createdAt: -1 });
